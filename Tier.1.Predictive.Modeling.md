...
# formula is the formula of the linear model expressed in the following syntax: 'target variable' ~ function (predictor variables).
# for example, target variable ~ pred.var.1+pred.var.3 or target variable ~ pred.var.1+pred.var.2+pred.var.3 and so on.
# In the case of formula = 'target variable ~ . ', '.' means a linear model involving all the available predictor variables.  
# algae.lm1 represents the object that contains information about the fitted linear model.
...
3. The summary also provides the value of estimated adjusted regression coefficient (adj.R2) which is useful to know how much of the data is explained by the model. 
4. Variable significance is indicated by * (p-value<0.05) or ** (p-value<0.01) or *** (p-value <0.001);   
5. p-value was arrived at based on t-value = Estimate/Std.Error and we can see that a few variables such as size(small) and No3 significantly contribute to the linear model. This step of variable selection, therefore, helps to identify these significant variables that can then be used to construct the machine learning mathematical model in Step 3.2
...
Additional thoughts:
Another useful piece of information that can be analyzed from the model object is the residual data generated from the model. Residual data represents how much a mathematical model's predicted value of the target variable differs from the actual value of the target variable. Using the residual information, one can infer information such as whether there is normality in the data as well as detecting outlier observations in the dataset.

```r
par(mfrow = c(2, 2))  # this command creates a 2 by 2 spread for the four output images generated by the below plot command 
plot(algae.lm1)
```
...
Now that outliers have been eliminated from the data, one of the first things to do is to split the full dataset into test dataset and training dataset because:  
(i) Training dataset is used for variable selection and mathematical model development  
(ii) Test dataset should be reserved purely for final validation of the generated machine learning model to prevent any learning bias creeping into the model
